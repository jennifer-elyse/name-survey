...
const ADMIN_PASSWORD = `U0GhUMMIeEBDSpS5nzVDBJa9qdkZCMz-`; //crypto.randomBytes(ADMIN_PASSWORD_LENGTH).toString('base64url');
...
app.post("/api/vote", (req, res) => {
	...
	if (state.participantLookup[normalizedParticipant]) {
		return res
			.status(409)
			.json({ error: "That participant has already voted." });
	}
...
app.get("*", (req, res, next) => {
    ...
    <html>
      ...
      <body>
        ...
        <p>Build the frontend with <code>npm run build</code> or use <code>npm run dev</code> for the Parcel client and API together.</p>
      </body>
    </html>
  `);
});
